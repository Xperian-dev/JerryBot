...
db_file = f"{directory}/files/reactionjerry.db"
db = database.Database(db_file)

#Begin RL Role Section
class Locks:
    def __init__(self):
        self.locks = {}
...
greet_response = [
    "Salutations.",
    "Ahoy, m'boy.",
    "Jeez, they'll let anybody in here, huh?",
    "Ayy, wassup?",
    "Suh dude!",
    "What did you just say to me? No really, I can't hear you without my glasses on.",
    "https://media1.tenor.com/images/2c2a669921c193995ffa47edf599a028/tenor.gif",
    "https://media1.giphy.com/media/Nx0rz3jtxtEre/giphy.gif",
    "I'm only here for the illegal underground grandma karaoke bars.",
...
async def on_message(message):
    ...
    elif any([keyword in msg for keyword in ('Oops', 'oops', 'Oopsy', 'oopsy')]):
        await message.channel.send(random.choice(gravy_words))

    elif any([keyword in msg for keyword in ('Thanks Jerry', 'thanks jerry', 'Thanks jerry', 'thanks Jerry')]):
        await message.channel.send(file=discord.File(f"{directory}/files/yee.png"))

    elif any([keyword in msg for keyword in ('Sand', 'sand', 'Sandwich', 'sandwich', 'Sandpaper', 'sandpaper', 'Sandy', 'sandy')]):
        await message.channel.send("I don't like sand. It's coarse, and rough, and irritating, and it gets all up in my *Nabooty*.")
...
async def new(ctx):
    if isadmin(ctx.message.author, ctx.guild.id):
        sent_initial_message = await ctx.send(
            "Welcome to the JerryBot role creation program. Please provide the required information once requested. If you would like to abort the creation, do not respond and the program will time out."
        )
        rl_object = {}
        cancelled = False
        ...
        if not cancelled:
            ...
            try:
                ...
                await ctx.author.send(
                    "Reaction role creation failed, you took too long to provide the requested information."
                )
                cancelled = True
            finally:
        ...
        if not cancelled:
            sent_limited_message = await ctx.send(
                "Would you like to limit users to select only one of the roles at a given time? Please react with a 🔒 to limit users or with a ♾️ to allow users to select multiple roles."
            )
                    ...
                    while True:
                        ...
                        try:
                            ...
                            error_messages.append(
                                (
                                    await ctx.send(
                                        f"This message already has a reaction role instance attached to it, consider running `{prefix}edit` instead."
                                    )
                                )
                            )
                except asyncio.TimeoutError:
        ...
        if not cancelled:
            ...
            try:
                ...
                await ctx.send(
                    f"The requested message already has a reaction role instance attached to it, consider running `{prefix}edit` instead."
                )
                return
            ...
            if isinstance(r, Exception):
                await system_notification(
                    ...
                    f"Database error when creating reaction-role instance:\n```\n{r}\n```",
                )
                return
...
async def hlp(ctx):
    if isadmin(ctx.message.author, ctx.guild.id):
        ...
        await ctx.send(
            ...
            f"{botname} is running version {__version__} of JerryBot."
        )
